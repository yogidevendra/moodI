 ...
 */
package com.datatorrent.io.db.jdbc;
 ...
 * API.<br/>
 * This operator by default uses the {@link #fieldInfos} and the table name to
 * construct the sql query.<br/>
 *
 * A user can provide there own query by setting the {@link #query} property
 * which takes precedence.<br/>
 *
 * For eg. user can set the query property to a complex one : "select x1, x2
 * from t1, t2 where t1.x3 = t2.x3 ;"<br/>
 *
 * This implementation is generic so it uses offset/limit mechanism for batching
 * which is not optimal. Batching is most efficient when the tables/views are
 * indexed and the query uses this information to retrieve data.<br/>
 * This can be achieved in sub-classes by overriding
 * {@link #queryToRetrieveData()} and {@link #setRuntimeParams()}.
 *
 * This class is an exact copy from malhar with the addition of getOutputPort().
 * Also, getOutputPort() in emitTuples() instead of the variable outputPort
 *
    ...
    implements Operator.ActivationListener<Context.OperatorContext>
{
  ...
  public void setup(Context.OperatorContext context)
  {
    ...
    try {
      ...
      preparedStatement = store.getConnection().prepareStatement(queryToRetrieveData());
  ...
  /**
   * @return fetch size which is the number of rows retrieved from the database
   *         in a window.
   */
  ...
  /**
   * Sets the fetch size which is the number of rows retrieved from the database
   * in a window.
   *
   * @param fetchSize
   *          number of rows retrieved from db in a window.
   */
  ...
  /**
   * This sets the direction used in processing a result set. It allows the JDBC
   * driver to optimize its processing. Refer
   * {@link PreparedStatement#setFetchDirection(int)}
   *
   ...
   *
   * @param mysqlSyntax
   *          true if it is mySql; when false oracle syntax is used.
   */
  ...
  /**
   * Function to initialize the list of {@link FieldInfo} externally from
   * configuration/properties file. Example entry in the
   * properties/configuration file: <property>
   * <name>dt.operator.JdbcPOJOInput.fieldInfosItem[0]</name> <value> {
   * "columnName":"ID", "pojoFieldExpression": "id", "type":"INTEGER" } </value>
   * </property>
   * 
   * @param index
   *          is the index in the list which is to be initialized.
   * @param value
   *          is the JSON String with appropriate mappings for
   *          {@link FieldInfo}.
   */
