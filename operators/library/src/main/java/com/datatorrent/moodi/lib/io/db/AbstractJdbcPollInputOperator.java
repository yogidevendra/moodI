    ...
    ActivationListener<OperatorContext>, Partitioner<AbstractJdbcPollInputOperator<T>>
{
  ...
  public void activate(OperatorContext context)
  {
    ...
    if (largestRecoveryWindow == Stateless.WINDOW_ID
        || context.getValue(OperatorContext.ACTIVATION_WINDOW_ID) > largestRecoveryWindow) {
   ...
   */
  @Override
  public Collection<Partition<AbstractJdbcPollInputOperator<T>>> definePartitions(
  ...
  public void partitioned(
      Map<Integer, Partition<AbstractJdbcPollInputOperator<T>>> partitions)
  {
  ...
  private int getRecordsCount() throws SQLException
  {
    ...
    if ((getWhereCondition() != null) && !(getWhereCondition().isEmpty())) {
  ...
  protected String buildRangeQuery(int offset, int limit)
  {
    ...
    if (getColumnsExpression() != null) {
      ...
      sqlQuery = create.select(columns).from(getTableName()).where(condition)
